...
namespace Tourista.API.Controllers
{
    public class HotelController : BaseApiController
    {
        ...
        [Route("api/Hotels/GetAllOnlineRelatedHotelsById", Name = "GetAllOnlineRelatedHotelsById")]
        [HttpGet]
        public IHttpActionResult GetAllOnlineRelatedHotelsById(long hotelId, int page = Page, int pagesize = PageSize)
        {
            PagedResultsDto hotelObj = _hotelFacade.GetAllOnlineRelatedHotelsById(hotelId,page, pagesize, TenantId);
            var data = Mapper.Map<List<HotelModel>>(hotelObj.Data);
            if (data != null)
                foreach (var item in data)
                {
