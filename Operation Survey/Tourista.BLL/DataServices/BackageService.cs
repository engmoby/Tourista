...
namespace Tourista.BLL.DataServices
{
    public class BackageService : Service<Backage>, IBackageService
    {
        ...
        {
            var query = Queryable().Where(x =>   x.CityId== cityId).OrderBy(x => x.BackageId);
            PagedResultsDto results = new PagedResultsDto();
            results.TotalCount = query.Select(x => x).Count();
            var modelReturn = query.OrderBy(x => x.BackageId).ToList();
        ...
        { 
            var query = Queryable().Where(x => !x.IsDeleted && x.TenantId == tenantId  ).OrderBy(x => x.BackageId);
            PagedResultsDto results = new PagedResultsDto();
            results.TotalCount = query.Select(x => x).Count(); 
        ...
        {
            var query = Queryable().Where(x => !x.IsDeleted && x.BackageId != BackageId && (x.TenantId == tenantId)).OrderBy(x => Guid.NewGuid()).Take(10);
            PagedResultsDto results = new PagedResultsDto();
            results.TotalCount = query.Select(x => x).Count();
            var modelReturn = query.OrderBy(x => x.BackageId).Skip((page - 1) * pageSize).Take(pageSize).ToList();
            results.Data = Mapper.Map<List<Backage>, List<BackageDto>>(modelReturn);
